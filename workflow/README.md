...
orchestrator (GLM-5.3-Flash)
        │
        ├── coder sub-agents (GPT-5.6 Luna, parallel by disjoint ownership)
        │
        ├── reviewer sub-agents (GPT-5.6 Luna, parallel, read-only)
        │
        └── PR agent (GPT-5.6 Luna) ──> optional `gh pr create`
```
